...
<div class="max-w-[560px] mx-auto w-full px-1 sm:px-0">
    ...
    <form method="POST" action="{{ route('reports.update', $report) }}" class="mt-5 grid gap-6">
        ...
        <div class="grid gap-2">
            ...
            <input name="title" minlength="3" maxlength="255" value="{{ old('title', $report->title) }}" class="w-full min-h-[44px] text-[16px] sm:text-[13.5px] bg-white border border-[#e6e9e1] rounded-xl px-3.5 py-2.5 text-[#1a2e1f] placeholder:text-[#b0bab2] focus:outline-none focus:border-[#0e6a38] focus:ring-2 focus:ring-[#0e6a38]/10 transition">
        ...
        <div class="grid gap-2">
            ...
            <input type="date" name="report_date" max="{{ date('Y-m-d') }}" value="{{ old('report_date', $report->report_date->toDateString()) }}" class="w-full min-h-[44px] text-[16px] sm:text-[13.5px] bg-white border border-[#e6e9e1] rounded-xl px-3.5 py-2.5 text-[#1a2e1f] focus:outline-none focus:border-[#0e6a38] focus:ring-2 focus:ring-[#0e6a38]/10 transition">
        ...
        <div class="flex flex-col-reverse sm:flex-row gap-2 pt-1">
            <a href="{{ route('reports.show', $report) }}" class="w-full sm:w-auto inline-flex items-center justify-center px-5 min-h-[44px] rounded-xl border border-[#e6e9e1] bg-white text-[13px] font-bold text-[#6b7a6e] hover:border-[#0e6a38] hover:text-[#0e6a38] transition">{{ __('ui.back') }}</a>
            <button class="w-full sm:w-auto sm:ms-auto inline-flex items-center justify-center px-7 min-h-[44px] rounded-xl bg-[#0e6a38] hover:bg-[#0a4d28] text-white font-bold text-[13px] transition shadow-sm">{{ __('ui.save') }}</button>
        </div>
    </form>
</div>
<style>
