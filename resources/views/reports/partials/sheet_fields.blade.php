{{-- حقول الورقة الرسمية — مشترك بين معاينة صفحة التقرير وصفحة الطباعة --}}
@if($sheet['fields'])
...
@foreach($shNotes as $i => $n)
    ...
    {{--Stored-only: النسخة المترجمة المحفوظة مسبقاً (أو المصدر) — ZERO Gemini هنا إطلاقاً.--}}
...
@if(trim((string) $report->recommendations) !== '')
    <div class="sh-note sh-fit" data-pitch="{{ $sheet['reco_pitch'] }}" style="left:{{ $shCfg['reco_rect']['x'] }}%;top:{{ round($shCfg['reco_first'] - 1.3, 2) }}%;width:{{ $shCfg['reco_rect']['w'] }}%;height:{{ round($shCfg['reco_bottom'] - $shCfg['reco_first'] + 0.9, 2) }}%;">{{ l10n_text('report', $report->id, 'recommendations', $report->recommendations) }}</div>
@endif
