// Print Modal — extracted from print_modal.blade.php
// Uses window.PRINT_T config (set by inline Blade script)
...
function renderAttachmentsSelector() {
    ...
    currentAttachmentsState.forEach((item, index) => {
        ...
        const imgWord = String(PRINT_T.photoBadge || '').replace(/^—\s*/, '') || PRINT_T.imageSingle;
...
function buildA4PageHtml(note, pageItems, pageIndex, totalPages, isLastPage, withDesc = true) {
    ...
    if(pageItems.length===3){
        ...
        console.log('[N3 TRACE][GEOMETRY_CHECK]', {
