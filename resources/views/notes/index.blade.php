...
<div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-5">
    <div>
        <h1 class="text-xl font-extrabold text-ink-800 leading-tight">{{ __('ui.observations_section') }}</h1>
    </div>
    ...
    @if(isset($observerUser) && $observerUser)
        <div class="mt-4 p-4 bg-white border border-surface-300 rounded-xl flex items-center gap-3 shadow-sm">
            @if($observerUser->avatar_url)
                <a href="{{ route('profile.showUser', $observerUser->id) }}" class="shrink-0 hover:opacity-80 transition" aria-label="{{ __('ui.profile_aria') }}"><img src="{{ $observerUser->avatar_url }}" alt="{{ $observerUser->name }}" class="w-10 h-10 rounded-xl object-cover border border-surface-300 shrink-0"></a>
            @else
            ...
            <div class="min-w-0 flex-1">
                <div class="text-sm font-extrabold text-ink-800">{{ $observerUser->name }}</div>
            </div>
            <a href="{{ route('notes.index', request()->except(['observer','user','page'])) }}" class="shrink-0 px-3 py-2 rounded-lg bg-surface-100 border border-surface-300 text-ink-600 text-xs font-bold hover:bg-white transition">{{ __('ui.clear_filter') }} ✕</a>
        </div>
    @endif
            ...
            <form method="GET" action="{{ route('notes.index') }}">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 mt-3">
                    ...
                    <div class="flex gap-2 items-end">
                        ...
                        <a href="{{ route('notes.index', request()->has('status') ? ['status' => request('status')] : []) }}" class="inline-flex items-center justify-center px-3 py-2 rounded-lg border border-[#e6e9e1] text-[#737373] font-medium text-sm hover:bg-[#f5f7f5] transition">{{ __('ui.clear') }}</a>
                    </div>
                </div>
            </form>
        </div>
    ...
    <div class="bg-surface-50 rounded-xl border border-surface-300 p-10 text-center">
        ...
        <div class="mt-5 flex items-center justify-center gap-2">
            @if(request()->hasAny(['date','floor_number','camera_number']))
                <a href="{{ route('notes.index', request()->has('status') ? ['status' => request('status')] : []) }}" class="px-4 py-2 rounded-lg border border-[#e6e9e1] text-[#525252] font-medium text-sm hover:bg-[#f5f7f5] transition">{{ __('ui.clear_filters') }}</a>
            @endif
            @if($isMonitor)
                <a href="{{ route('notes.create') }}" class="px-4 py-2 rounded-lg bg-[#0e6a38] text-white font-bold text-sm hover:bg-[#0a4d28] transition">+ {{ __('ui.new_note') }}</a>
            @endif
        </div>
    </div>
@else
            ...
            <div class="note-row grid grid-cols-[1fr_auto_auto] gap-4 items-center px-5 py-3.5 border-b border-surface-300 last:border-b-0 cursor-pointer hover:bg-surface-50 transition-colors" onclick="openModal('detail-{{ $note->id }}')" data-i18n-entity="note" data-i18n-id="{{ $note->id }}">
                ...
                <div class="min-w-0">
                    <div class="flex items-center gap-2 flex-wrap">
                        ...
                        <span class="hidden md:inline text-[11px] text-ink-400/80 font-mono" dir="ltr" title="{{ __('ui.created_at_label') }}">{{ $note->created_at->format('Y-m-d H:i') }}</span>
                    </div>
                    <p class="mt-1 text-sm font-medium text-ink-700 line-clamp-1 leading-relaxed" data-i18n-field="description">{{ \Illuminate\Support\Str::limit(l10n_text('note', $note->id, 'description', $note->description), 90) }}</p>
