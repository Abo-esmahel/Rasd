...
<head>
    ...
    <style>
        *{font-family:'Cairo','Segoe UI',sans-serif} body{-webkit-font-smoothing:antialiased;text-rendering:optimizeLegibility;overflow-x:hidden}
        ...
        html.dark .bg-\[\#f1f3f0\]{ background-color:#1e2320 !important; }
        html.dark .bg-\[\#f6f7f5\]{ background-color:#2a302b !important; }
        html.dark .bg-white{ background-color:#252b26 !important; }
        html.dark .border-\[\#e6e9e1\]{ border-color:#2e352e !important; }
        html.dark .border-\[\#dde8df\]{ border-color:#2e352e !important; }
        html.dark .text-\[\#1a2e1f\], html.dark .text-\[\#0f1e14\]{ color:#e7ece5 !important; }
        html.dark .text-\[\#4a5a4f\], html.dark .text-\[\#2a3d2f\]{ color:#a8b5a8 !important; }
        html.dark .text-\[\#6b7a6e\], html.dark .text-\[\#5f7568\]{ color:#9bb0a0 !important; }
        html.dark .text-\[\#7a8a80\], html.dark .text-\[\#94a8a0\]{ color:#8fa090 !important; }
        html.dark .bg-\[\#e6f2eb\]{ background-color:#1e3322 !important; }
        html.dark .text-\[\#0e6a38\]{ color:#4ade80 !important; }
        html.dark input, html.dark textarea{ background-color:#2a302b !important; border-color:#2e352e !important; color:#e7ece5 !important; }
        html.dark input::placeholder, html.dark textarea::placeholder{ color:#7e8e7e !important; }
        html.dark .hover\:bg-\[\#eceee9\]:hover{background-color:#2e352e !important}
        html.dark .hover\:bg-white:hover{background-color:#2e352e !important}
        html.dark .hover\:bg-\[\#f6f7f5\]:hover{background-color:#2e352e !important}
    </style>
</head>
<body class="min-h-screen bg-[#eceee9] flex flex-col selection:bg-[#0e6a38] selection:text-[#fdfcfa] overflow-x-hidden">
    ...
    <div class="h-[4px] w-full flex shrink-0">
        ...
        <div class="flex-1 bg-white border-y border-black/5"></div>
            ...
            <div class="flex flex-col justify-center min-w-0">
                ...
                <div class="bg-[#fdfcfa] rounded-[24px] shadow-sm border border-[#e6e9e1] overflow-hidden">
                    ...
                    <div class="p-6 sm:p-8 lg:p-9">
                        ...
                        @if($errors->any())
                            <div class="mb-6 flex gap-3 p-4 bg-[#fef2f2] border border-[#fecaca] rounded-2xl text-[#991b1b]" role="alert">
                                <div class="shrink-0 w-9 h-9 rounded-xl bg-[#c41e1e] text-white flex items-center justify-center">
    ...
    <script>
        ...
        (function(){
            ...
            function sync(){ const d=html.classList.contains('dark'); document.querySelectorAll('.sun-i').forEach(e=>e.classList.toggle('hidden',d)); document.querySelectorAll('.moon-i').forEach(e=>e.classList.toggle('hidden',!d)); }
            function applyStoredTheme(){ try{ var s=localStorage.getItem('theme')||localStorage.getItem('rasd_theme'); var m=window.matchMedia('(prefers-color-scheme: dark)').matches; var wantDark=s==='dark'||(!s&&m); if(wantDark) html.classList.add('dark'); else html.classList.remove('dark'); sync(); }catch(e){} }
            sync();
